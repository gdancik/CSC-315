...
count.heads <- function(n) {
  ...
  prop.heads = sum(tosses == "H") / length(tosses)
  return(prop.heads)
}


p.hat = replicate(5000, count.heads(100))
...
hist.norm(p.hat, mu, sigma / sqrt(n), main = "Proportion of Heads in 100 Tosses",
          xlab = "proportion of heads (p.hat)")
...
##################################################
# p-value is area in unshaded region
#################################################
...
###################################################
## compare to p-value from simulations
###################################################
p3.a = 2*(sum(p.hat >= .62) / length(p.hat))
p3.b = (sum(p.hat >= .62) + sum(p.hat <= 0.38)) / length(p.hat)
