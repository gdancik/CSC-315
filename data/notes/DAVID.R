...
##############################################################################
# the lines below uses the getGEO function from GEOquery to 
# download the platform data 
# pl <- getGEO(platform)
# pl <- Table(pl)
##############################################################################

# for this example, we will load this data from a previous workspace
...
## go toDAVID (https://david.ncifcrf.gov/)
## select Functional Annotation -> Upload gene list, 
## set identifier to OFFICIAL_GENE_SYMBOL, select Gene List.
## if necessary, set the Background species (to Homo sapiens in this case)
...
# based on DAVID?
###########################################################################
genes <-c("ASF1B","ASPM","C18orf24","CDKN3","CENPM","DLGAP5",
           "DTL","KIAA0101","KIF20A","MCM10","ORC6L","PRC1",
           "RAD51","RAD54L","RRM2")
