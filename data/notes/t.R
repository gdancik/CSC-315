...
ylim = c(0, max(pz, t=pt1, pt5, pt30))

plot(x, pz, type = "l", main = "Normal and Student's t Distributions", xlab = "x", 
     ylab = "density", col = colors[1])
colors = c("red", "skyblue", "darkblue", "darkgreen")
lines(x, pt1, col = colors[2])
lines(x, pt5, col = colors[3])
lines(x, pt30, col = colors[4])

legend = c("normal", "t, df=1", "t, df=5", "t, df=30")
legend("topleft", legend = legend, col = colors, lty = 1)
...
par.orig = par(mfrow = c(2,1), mar = c(2,2,1,1))
hist.Z(t, df = NULL, breaks = 40, main = "Histogram of t")
hist.Z(t, df = n-1, breaks = 40, main = "Histogram of t")
par(par.orig)
...
plot(x, dt(x, df = n - 1), type = "l", xlab = "t", ylab = "density",
     main = main)
abline(v = t, col = "red")
