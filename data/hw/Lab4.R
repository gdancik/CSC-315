...
#4. What is the probability that it is an ace OR a 4? Note: you
#   may want to write list the outcomes that are either an 
#   ace or a 4, to make sure you are counting correctly.
...
#   two dice and then calculates the sum, and checks whether the sum
#   is 7. Then use the 'replicate' function to find the empirical 
#   probability based on 5000 experiments.



# Definition: A probability distribution of a discrete random variable 
# gives the probability for each value that the variable can take. 
...
#   set of 3 coin flips. Then create a relative frequency table for 
#   X = the number of heads in 3 coin tosses. This relative frequency
#   table is the probability distribution for X! Also create a bar 
#   graph of the relative frequencies, making sure to appropriately 
#   label the x-axis, y-axis, and title of the graph. Note: your 
#   results should match the theoretical probabilities which are 
#   P(X=0) = 0.125, P(X=1) = 0.375, P(X=2) = 0.375, and P(X = 3) = 0.125
...
#   replacement (repeats.allowed = FALSE) because we cannot include
#   the same card twice in one hand. We also specify 'set = FALSE' to 
#   allow for duplicate values in the deck vector. Finally, each 
#   combination is equally likely, so classical probability can be used.
